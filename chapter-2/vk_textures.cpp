...
bool vkutil::create_image(VulkanEngine& engine, int texWidth, int texHeight, AllocatedImage& outImage)
{

	unsigned int* pixel_ptr = new unsigned int[texWidth * texHeight];
	...
	VkImageCreateInfo dimg_info = vkinit::image_create_info(image_format, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, imageExtent);
	...
	std::cout << "Texture created succesfully " << texWidth << "x" <<texHeight << std::endl;
